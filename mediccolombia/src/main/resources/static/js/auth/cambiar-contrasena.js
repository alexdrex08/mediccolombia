async function verificarPaso1() {
    ...
    if (!token) { mostrarError('Ingresa el token de verificación.'); return; }

    try {
        // Verificar que el documento existe en BD
        console.log("verificado documento:" + identificacion);
        const res = await fetch(
            `/auth/api/verificar-token-cambio?identificacion=${encodeURIComponent(identificacion)}&token=${encodeURIComponent(token)}`,
        ...
        if (res.status === 403) {
            mostrarError('Token de verificación inválido. Contacta al administrador.');
            return;
        }
        if (res.status === 404) {
...
function irPaso(num) {
    ...
    const subtitulos = {
        ...
        2: 'Paso 2 de 2 — Cambia tu contraseña'
    };
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    form.addEventListener('submit', function (e) {
        ...
        // Verificar que la nueva contraseña cumpla con el mínimo
        if (nueva.length < 6) {
            e.preventDefault();
    ...
    document.getElementById('inputNueva').addEventListener('input', function () {
        ...
        if (this.value && this.value === actual) {
