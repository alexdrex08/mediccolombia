            ...
            .then(response => {
                ...
                if (!response.ok) {
                    if (response.status === 401) {
                        ...
                        mostrarAlerta('error', 'Tu sesión ha expirado. Serás redirigido al login.');
                        setTimeout(() => window.location.href = '/auth/login?expired', 1500);
                        return;
                    }
                ...
                function () {
                    ...
                    fetch(`/api/usuario/${userId}/foto`, {
                        ...
                        .then(response => {
                            if (!response.ok) {
                                if (response.status === 401) {
                                    mostrarAlerta('error', 'Sesión expirada. Redirigiendo...');
                                    setTimeout(() => window.location.href = '/auth/login?expired', 1500);
                                    return;
                                }
                                return response.text().then(text => {
